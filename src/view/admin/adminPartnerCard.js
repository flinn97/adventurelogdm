 ...
 */
export default class AdminPartnerCard extends Component {
  constructor(props) {
    super(props);
    ...
    return (
      <div style={{ height: "100%" }} >
        <div style={{ display: "flex", flexDirection: "row", justifyContent: "space-between", color: "black" }}>
          ...
          <h1>Payment Type</h1>
          <h1>Partner Start Date</h1>
          <h1>Total Payments</h1>
        </div>
        <div>
          {this.state.start &&
            <MapComponent app={app} name="user"
              filter={{ search: true, attribute: "partner" }}
              cells={["firstName", "lastName"]}
              linkOptions={{ cells: [0, 1], path: ["/partner/"] }} />}
        </div>
      </div>
    )
  }
