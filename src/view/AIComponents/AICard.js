...
sendMessage = async (state, dispatch) => {
  if (this.state.content.trim() !== "") {
      ...
      let newUserMessage = {
          ...
          visible: true,
          getJson: function () { return this; } // Ensures getJson() method exists
      };
      ...
      this.setState(prevState => ({
          ...
          let messages = await state.componentList.getList("aiMessage", assistantId, "assistantId");
          let visibleMessages = messages.filter(msg => msg.getJson().visible !== false);
  ...
  render() {
    ...
    let chatHistory = this.state.prevGens.filter(item => 
      item._id !== (this.state.currentAssistant ? this.state.currentAssistant.getJson()._id : null)
    );
    
    let messagesInThisChat = this.state.messageList.length;
    
    return (
      <div ref={this.startRef} style={{
      ...
      <div style={{width:"fit-content", transition: "all .8s ease",
        right: 11, top: 11, position: "absolute", display:"flex", flexDirection:"row", gap:"11px", marginBottom: "2vh",}}>
        ...
        className='hover-btn' to={"ruleset/"} style={{
          ...
          textDecoration: "none", fontStyle: "italic", display: "flex", flexDirection: "column",
          fontWeight: "bold", letterSpacing: ".05rem",  padding: this.state.showHistory?"3px .8vw":"3px 8px", fontSize: "1vw", transition:"all .8s ease",
          border:this.state.showHistory?"1px solid rgb(192, 189, 11)":styles.buttons.buttonAdd.border,
          background: this.state.showHistory?styles.colors.color8 + "22":styles.colors.color7 + "aa",
        }}>
          <img src={history} draggable={false} style={{ transition:"all .8s ease", width: this.state.showHistory?"0px":"1.5vw", opacity:this.state.showHistory?"0":"1" }} />
          <img src={cancel} draggable={false} style={{ transition:"all .8s ease", width: this.state.showHistory?"1.5vw":"0px", opacity:this.state.showHistory?"1":"0" }} />
        </div>}
        ...
        className='hover-btn' to={"ruleset/"} style={{
          ...
          fontWeight: "bold", letterSpacing: ".05rem", padding: this.state.showHistory?"":"3px 8px", fontSize: "1vw", transition:"all .8s ease",
          opacity:this.state.showHistory?"0":"1", pointerEvents:this.state.showHistory?"none":"",
        }}>
          <img 
          draggable={false}
          src={gear} 
          style={{ width: this.state.showHistory?"5px":"1.5vw", opacity:this.state.showHistory?"0":"1", transition:"all .8s ease", }} />
        </Link>
      </div>

        {/* {Old Convos} */}
      {(chatHistory.length > 0) &&
      <div style={{ width:"81.5vw", height:this.state.showHistory?"":"0px",
        transition: "all .8s ease", overflow:"hidden",  marginLeft: "22px",  marginRight: "22px",
        background: styles.colors.color8 + "22", padding: this.state.showHistory?"24px 28px":"", 
        borderRadius: "11px", marginTop: this.state.showHistory?"-1px":"40px", 
      pointerEvents:this.state.showHistory?"":"none", }}>
        <div style={{display:"flex", flexDirection:"row"}}>
          <img 
          draggable={false}
          src={history} 
          style={{ width: this.state.showHistory?"1.33vw":"1vw", height:"1.33vw" ,transition:"all .8s ease", marginRight:"29px", }} />
        <div style={{ color: styles.colors.color3, fontSize: this.state.showHistory?"1.33vw":"1vw", transition:"all .8s ease" }}>Previous Generations</div>
        </div>
        
          <MapComponent app={app} list={chatHistory}
            reverse={true}
            cells={[{
              type: "attribute",
              name: "name",
              class: "hover-text-highlight-blue",
              style: { opacity:this.state.showHistory?"1":"0", transition:"opacity .3s ease-in",
                color: "white", fontSize: "1vw", paddingLeft: "22px", overflow:"hidden", marginLeft:"22px",
                textDecoration: "1px underline " + styles.colors.color8, textUnderlineOffset: "2px", maxHeight:"1vw",
              }, func: async (obj) => {
              ...
              style:{cursor:"pointer", filter:"brightness(88%)", width:"31px", marginLeft:"2vh", marginBottom:"-4px", padding:"1px 4px"}, 
        ...
        <div style={{ display:"flex", flexDirection:"row", marginTop: "4vh", marginBottom: "12px", justifyContent:"space-between", paddingRight:"2vw" }}>
          <textarea className='textareafixed'
            value={this.state.content}
            placeholder={messagesInThisChat>0?'Continue conversation...':"Start a new conversation..."}
            style={{
